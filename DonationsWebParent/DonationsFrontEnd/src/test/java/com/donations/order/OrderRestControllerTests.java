...
public class OrderRestControllerTests {
	...
	@Test
	@WithMockUser("nguyentrunghai2106@gmail.com")
	public void testSendOrderReturnRequestFailed() throws Exception {
		Integer orderId = 99;
		OrderReturnRequest returnRequest = new OrderReturnRequest(orderId, "Test", "Test Notes");
	...
	public void testSendOrderReturnRequestSuccessfull() throws Exception {
		Integer orderId = 29;
		OrderReturnRequest returnRequest = new OrderReturnRequest(orderId, "Mua nhầm sản phẩm", "Vui lòng hoàn trả tiền cho tôi");
		String requestURL = "/orders/return";
		mockMvc.perform(post(requestURL).with(csrf()).contentType("application/json")
				.content(objectMapper.writeValueAsString(returnRequest))).andExpect(status().isOk())
				.andDo(print());
	}
}
